...
In order to solve this, we need to understand that the minima of f(x) will occur when both f(x) and h(x) are tangent to each other. &Implies; **&nabla;<sub>x</sub> f = &lambda; (&nabla;<sub>x</sub> h)**

therefore, in order to solve this problem, we define a new Langragian function, &#8466;(x,&lambda;) = f(x) + &lambda;(h(x))
...
In this case, the Langrangian function would be : &#8466;(x,&lambda;<sub>1</sub>,&lambda;<sub>2</sub>,...,&lambda;<sub>n</sub>) = f(x) + &Sigma;&lambda;<sub>i</sub> h<sub>i</sub>(x)
...
New Langrangian function - &#8466;(x,&alpha;) = f(x) + &alpha;(g(x))
...
The Langragian function in this case would be, &#8466;(x,&lambda;<sub>1</sub>,&lambda;<sub>2</sub>,...,&lambda;<sub>n</sub>,&alpha;<sub>1</sub>,&alpha;<sub>2</sub>,...,&alpha;<sub>m</sub>) = f(x) + &Sigma;&lambda;<sub>i</sub> h<sub>i</sub>(x) + &Sigma;&alpha;<sub>j</sub> g<sub>j</sub>(x)
...
The 2 values d and p are equal when they staisfy a set of condition called **KKT conditions**. Following are the condition,
...
The Notations used for SVM is slightly different from that of logistic regression where we used 0 and 1 for representing success and failure. SVM used {-1, +1} for differetiating between classes. This technique uses vector approach and identifies a plane that separates both classes from each other.
...
This is another metric called geometric margin, &gamma;<sup>(i)</sup> = <sup>&gamma;&#770; <sup>(i)</sup></sup> &frasl; <sub>||w||</sub>

So, our objective becomes, &gamma; = min &gamma;<sup>(i)</sup> (choose the plane such that the distance between plane and the closest data point is minimum)
...
In order to solve our problem, we will be using the Langragian Method described above in the pre-requiste section. Here, we have an objective function we want to minimize subject to an inequality constraint.
...
Therefore, we define a langrangian function, **&#8466;(w,b,&alpha;) =  <sup>||w||<sup>2</sup></sup> &frasl; <sub>2</sub> - &Sigma;<sub>i</sub><sup>m</sup> &alpha;<sub>i</sub> [y<sup>(i)</sup>(w<sup>T</sup>x<sup>(i)</sup> + b) - 1]**

Conditions to minimize the langrangian is, &nabla;<sub>w</sub> L = 0 and &nabla;<sub>b</sub> L = 0

Applying these conditions, we get, w = &Sigma;<sub>i</sub><sup>m</sup> &alpha;<sub>i</sub> y<sup>(i)</sup> x<sup>(i)</sup> and &Sigma;<sub>i</sub><sup>m</sup> &alpha;<sub>i</sub> y<sup>(i)</sup> = 0
...
Also, replacing the value of w in our prediction function, w<sup>T</sup>x + b = (&Sigma;<sub>i</sub><sup>m</sup> &alpha;<sub>i</sub> y<sup>(i)</sup> x<sup>(i)</sup>)x + b

&Implies; w<sup>T</sup>x + b = &Sigma;<sub>i</sub><sup>m</sup> &alpha;<sub>i</sub> y<sup>(i)</sup> (x<sup>(i)</sup>)<sup>T</sup>x + b
...
Primal Form of the Langrangian function, &theta;<sub>p</sub> = max<sub>&alpha;</sub> &#8466;(w,b,&alpha;) = f(w,b)

so, objective : min<sub>w,b</sub>  f(w,b) (This is again not solvable, So, we resort to Dual form of the problem)

and Dual Form of the Langrangian function, &theta;<sub>d</sub> = min<sub>w,b</sub> &#8466;(w,b,&alpha;)

Let's solve the Langragian first by using value of w obtained above in order to calculate &theta;<sub>d</sub>

&#8466;(w,b,&alpha;) =  <sup>||w||<sup>2</sup></sup> &frasl; <sub>2</sub> - &Sigma;<sub>i</sub><sup>m</sup> &alpha;<sub>i</sub> [y<sup>(i)</sup>(w<sup>T</sup>x<sup>(i)</sup> + b) - 1]

&Implies; &#8466;(w,b,&alpha;) = <sup>(&Sigma;<sub>i</sub><sup>m</sup> &alpha;<sub>i</sub> y<sup>(i)</sup> x<sup>(i)</sup>)(&Sigma;<sub>j</sub><sup>m</sup> &alpha;<sub>j</sub> y<sup>(j)</sup> x<sup>(j)</sup>)</sup> &frasl; <sub>2</sub> - &Sigma;<sub>i</sub><sup>m</sup> &alpha;<sub>i</sub> [y<sup>(i)</sup> { (&Sigma;<sub>j</sub><sup>m</sup> &alpha;<sub>j</sub> y<sup>(j)</sup> x<sup>(j)</sup>) x<sup>(i)</sup> + b} - 1]
